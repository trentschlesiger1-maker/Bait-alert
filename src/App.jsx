...
<style>{`*{box-sizing:border-box;margin:0;padding:0} ::-webkit-scrollbar{width:3px}::-webkit-scrollbar-thumb{background:#1a3a24} @keyframes fadeUp{from{opacity:0;transform:translateY(14px)}to{opacity:1;transform:translateY(0)}} .fu{animation:fadeUp 0.45s ease forwards} @keyframes spin{to{transform:rotate(360deg)}}.spin{animation:spin 0.9s linear infinite} @keyframes pulse{0%{transform:scale(0.9);opacity:0.7}100%{transform:scale(2.1);opacity:0}}.pulse{animation:pulse 2s ease-out infinite} @keyframes fillBar{from{width:0}to{width:var(--w)}}.bar{animation:fillBar 1.1s cubic-bezier(.2,1,.3,1) forwards} button:hover{opacity:0.82;transform:translateY(-1px)}button:active{transform:translateY(0)}`}</style>
