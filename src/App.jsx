...
export default function App() {
  ...
  var checkRouteRisk = async function() {
    ...
    try {
      var res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          model: "claude-sonnet-4-20250514",
          max_tokens: 600,
          system: "You are an expert on 1080 baiting zones in Australia. Return ONLY valid JSON, no markdown.",
          messages: [{ role: "user", content: "Assess 1080 bait risk for a road trip from " + routeStart + " to " + routeEnd + " in Australia. Return JSON: {\"overall\":\"LOW|MODERATE|HIGH|EXTREME\",\"summary\":\"2 sentence plain text summary\",\"segments\":[{\"name\":\"segment name\",\"risk\":\"level\",\"note\":\"brief note\"}]}" }]
        })
      });
      var data = await res.json();
      var block = data.content && data.content.find(function(b) { return b.type === "text"; });
      if (block) {
        var clean = block.text.replace(/```json|```/g, "").trim();
        setRouteResult(JSON.parse(clean));
      }
    } catch { setRouteResult({ overall: "UNKNOWN", summary: "Could not assess route. Please check your connection and try again.", segments: [] }); }
            ...
            <div style={{ maxWidth: 460, margin: "0 auto", display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <div style={card}>
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  <div>
                    ...
                    <input value={routeStart} onChange={function(e) { setRouteStart(e.target.value); }} placeholder="e.g. Perth WA" style={{ width: "100%", padding: "12px", borderRadius: 10, border: "1.5px solid " + border, fontSize: 14, color: textMain, background: bg, outline: "none" }} />
                  </div>
                  <div>
                    <div style={lbl}>Destination</div>
                    <input value={routeEnd} onChange={function(e) { setRouteEnd(e.target.value); }} placeholder="e.g. Broome WA" style={{ width: "100%", padding: "12px", borderRadius: 10, border: "1.5px solid " + border, fontSize: 14, color: textMain, background: bg, outline: "none" }} />
                  </div>
              ...
              {routeResult && (
                <>
                  ...
                  {routeResult.segments && routeResult.segments.map(function(seg, i) {
                    return (
                      <div key={i} style={card}>
                        <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                          <RiskBadge risk={seg.risk} />
                          <div>
                            <div style={{ fontSize: 14, fontWeight: "600", color: textMain, marginBottom: 4 }}>{seg.name}</div>
                            <div style={{ fontSize: 13, color: textSub, lineHeight: 1.6 }}>{seg.note}</div>
                          </div>
                        </div>
                      </div>
                    );
